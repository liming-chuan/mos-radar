# MOS Radar：安全边际雷达 V6.11.0

V6.11.0 为新模拟计划加入 **大盘MA50闸门与预计财报窗口检查**：证据缺失或未通过时保留候选及原因，不新增模拟入场。增加最新日线价格下的严格价值/质量复核标签，不采用旧S/A评级充当价格底。固定止损与1R后下一交易日上移止损分别记录，新排雷版本与旧记录不混算。

新增私人账户仓位测算：现金及挂单预留、单笔0.5%风险、单股10%、行业25%、总止损风险2%、最多5只，以及回撤5%降低新仓预算、10%暂停新仓。参数属于初始研究限制，不是20%回撤保证。**尚未提供账户资料，系统不输出真实买入股数。** 公开模拟不读取账户文件；运行方法、证据有效期与限制详见 [排雷和账户风险说明](docs/ENTRY_GUARDS_AND_ACCOUNT.md)。

V6.10.0 新增独立的 **强势回踩再转强** 策略（pullback-1），与原放量突破（breakout-1）共用日线数据，分别保存前瞻模拟记录。检查上升趋势中缩量回踩20日线、重新站上前日高点及量能确认；不要求创新高。下一交易日触发、最高追价、风险距离、扣费盈亏比和10交易日退出仍须满足。报告分策略展示预备名单、模拟计划与已结束样本，同一股票可在两个独立实验中出现，不能直接当成加仓建议。初始规则尚未经过收益验证，详见 [回踩策略说明](docs/PULLBACK_POLICY.md)。

V6.9.1 扩大机会发现：从通过基础和成交额门槛的股票中按行业交错、轮换检查，每轮最多300只，并优先复查上轮预备/触发名单。报告显示未覆盖和数据失败数量，将均线、相对强度、突破和量能逐项拆解，新增距离20日高点3%以内的强势预备名单。预备名单不生成模拟买入，原突破触发规则不变。

**每日定时邮件改为先发送短线简报，再完成全量价值扫描**，沿用原收件人和每日一次频率；完整价值报告保存在原工作流产物。简报使用最近盘后快照，不实时确认价格；GitHub任务自身延迟仍可能造成晚到。手动扫描邮件行为不变。新增覆盖、预备和排序是研究改进，不构成更高收益的证明。详见 [短线规则及运行说明](docs/SHORT_TERM.md)。

V6.9.0 新增独立的 **2–10个交易日短线实验池**：从公开股票池的高流动性股票中检查放量突破，生成下一交易日触发价、最高追价、止损参考、目标参考和10个交易日到期退出规则。独立盘后任务只抓价格、不重跑全量财报，逐笔记录计划生成后的模拟成交及估计费用。现有邮件展示短线计划和跟踪状态；美股开盘后报告改为延迟标题，打印不再误用手机卡片布局，持仓不再重复展开。严格价值筛选和估值模型仍保持 V6.8.0。

**短线收益尚未验证，当前不是实盘策略。** 没有仓位、账户净值和券商成本数据，不能计算或保证账户最大回撤约20%。首次运行没有历史模拟成绩；真实买卖还需核查财报日、公告、盘口和交易单位。规则、初始参数、局限与运行方法见 [短线实验池](docs/SHORT_TERM.md)。

V6.8.1 重排报告：首页先给结论和重点股票，观察池优先展示趋势通过项目，手机使用卡片，打印时重复表头并避免拆开股票行。时间转换为市场当地时间，缺失日期显示“未提供”。重复诊断和旧评级样本移入简短附录，完整数据保留在 CSV。只调整展示，估值模型仍为 V6.8.0，筛选规则与信号不变。详见 [报告可读性优化](docs/REPORT_READABILITY.md)。

V6.8.0 新增独立的优质合理估值研究池、趋势确认和前瞻记录，保留厚安全边际门槛。详见 [观察池规则](docs/QUALITY_TREND_WATCH.md)。

V6.7.4 将公告归母净利润与合并净利润分开存储，纠正旺旺补录的利润口径映射；真实冲突仍保留供应商值和公告值供核查。新增 SBC 整表缺失、项目缺失、部分年份缺失与历史不足分类。港股任务提前到香港时间 08:07，开盘后生成的原定盘前报告会明确标注“延迟”，并记录扫描开始、完成和耗时。见 [利润口径、SBC 与报告时效修复](docs/V6_7_4_BASIS_AND_TIMING.md)。

V6.7.3 增加年度现金流逐项缺失诊断和经审核的港交所公告补录通道：区分整张表未返回与 OCF/资本开支/SBC 具体年份缺失；补录只填空值，来源冲突、币种或期间异常会被拒绝，并使入场等待数据复核。补录变动会使港股缓存失效。已录入旺旺两年经 PDF 核对的历史收入、利润和现金流，SBC 未核实，保持缺失，不能将原始财报补录等同于入场证据已齐备。见 [财报证据补录与诊断](docs/STATEMENT_EVIDENCE.md)。

V6.7.2 增加港股最近十二个月财报的核验与回退，区分历史现金流为负和数据缺失；周期模型采用近期/多年现金流较低值及行业倍数，移除基准估值中额外的固定现金流折减，衰退折减仍在压力情景中执行。触发价现在列出价值折价、压力折价、现金流收益率三个上限及当前限制，较远候选改为简表。普通/年报/周期股的 35%/40%/50% 折价门槛不变。详见 [本次优化、参数影响与验证](docs/V6_7_2_PERIODS_AND_VALUATION.md)。实时港股抽查仍未取得十二个月报表，新路径尚不能视为数据覆盖问题已解决。

V6.7.1 根据实际美股/港股扫描报告修复：持续经营估值与 NCAV/有形净资产折价参考分开，年报末尾空列不再误报 SBC 证据缺失；扫描中断后的报告保存不会覆盖完整公开状态。报告首页改为严格入场统计，个股使用两列卡片，明确“仍需下跌多少”，并移除普通扫描中的空回放收益和误导性的“接近候选”列表。详见 [本次报告修复与验证](docs/SCAN_REPORT_FIXES.md)。

V6.7 新增严格入场观察区：用“相对保守估值的折价率 + 压力情景 + 现金流/偿债/数据门槛”显示触发上限、深折价观察价、还需等待的价格距离，以及新进入/退出事件。普通公司默认折价至少35%，周期股至少50%；数据不完整或风险未通过时不产生入场信号。以下 S/A/B 为兼容旧版的研究评级，不等于严格入场标准。

规则、计算修复、初始参数、验证限制和运行说明见 [严格入场观察规则](docs/ENTRY_POLICY.md)。这些阈值尚未经过收益验证。V6.7 首次运行会淘汰旧缓存，建议先少量股票、`DRY_RUN=true` 验证。`python -m unittest discover -s tests -v` 可运行离线回归测试。
...
- 周一至周五香港时间计划 08:07 启动：完整扫描港股候选池，保存 `data/results/hk_mos_latest.csv` 和 `reports/hk/latest_report.md`，并发送邮件；GitHub 调度可能延迟，09:30 之后生成会标注“延迟”
...
```text
旧 MoS / 潜在上涨空间 = (保守内在价值/股 - 当前股价) / 当前股价
新折价率 = (保守内在价值/股 - 当前股价) / 保守内在价值/股
```

保守内在价值按行业模型生成估值候选，并取持续经营方法中的最低有效估值；NCAV/有形账面折价单列为资产参考，不混入经营估值取最小值。只有资产参考可用时，转为专门复核，不生成经营型入场价。V6.7 不丢弃非正经营估值，也不把缺债务当零：

```text
Owner FCF = 经营现金流 - 资本开支 - 股权激励 SBC
最近/3年/5年 Owner FCF 中的较低值（非正保留为零）× 行业保守倍数
最近 Owner FCF × 10倍或更低
...
1. V6.6.2 使用 yfinance，适合个人研究原型，不适合机构级数据可靠性。
...
新增优质合理估值候选和趋势子集，保留厚安全边际规则。规则、退出复核及前瞻记录见 [观察池说明](docs/QUALITY_TREND_WATCH.md)。尚未验证收益，不保证20%回撤上限。
