...
- 新计划使用guards-1：大盘完整50日均线检查、入场前至最长10交易日的预计财报检查；未知不放行。
- 独立私人账户测算现金、风险、单股与行业限额、回撤减仓预算、每手股数及重复策略合并；缺资料不输出股数。
- 价值标签按新日线收盘重新核验，不使用旧S/A评级直接背书。
- 固定止损与BE_1R独立对照；至少完整持有一天后触及实际1R，下一交易日启用含估计成本的止损参考，不回改当天成交。
- 新增计划ID含排雷版本；旧未入场记录在入场前撤销，已发生的旧实验不回填财报证据或混入新对照。

# V6.10.0 - 2026-09-13

- 增加pullback-1：上升趋势、相对强度、缩量回踩20日线、收复前日高点和1.1倍量能确认；复用原行情请求。
- 以回踩低点下方0.1ATR为止损参考，2.5倍风险距离为目标情景；仍核验原1.5%–8%风险距离、扣费盈亏比和下一交易日有效期。
- 两策略分开去重、跟踪和统计，旧breakout-1记录原样继续；同股票双策略不合并为真实仓位。
- 同策略已有模拟持仓或已记录信号时，不再将重复计划显示为新入场机会，继续沿用原记录退出价格。
- 每个策略最多展示3个计划、4个预备；覆盖按股票计数，状态按策略项计数。

# V6.9.1 - 2026-09-13

- 从固定100只改为流动性合格池按行业交错轮换，每轮最多300只，优先复查预备名单；显示覆盖缺口。
- 拆分均线、相对强度、突破、量比条件；新增强势且距前20日高点3%以内的预备观察，不修改触发及模拟成交条件。
- 计划按相对强度、量比排序，预备按距突破位置排序，明确不是胜率排名。
- 原每日定时邮件先发轻量短线简报，随后继续价值扫描，避免重复发送；保留手动扫描行为。
- 既有breakout-1模拟记录连续保留，覆盖轮换另存进度；未新增回踩策略、事件日历或账户级回撤控制。

# V6.9.0 - 2026-09-12

- 新增独立的2–10交易日放量突破实验池、下一交易日价格计划及前瞻逐笔模拟记录。
- 交易日历处理节假日、半日市和收盘缓冲；盘后趋势计算可使用当天已完成K线。
- 模拟处理跳空、同日保守止损、估计费用/滑点、10交易日退出、数据缺失和公司行动暂停。
- 独立盘后价格工作流，最多100只新检查股票，继续跟踪既有模拟记录；无额外邮件或券商接入。
- 修复美股报告延迟标题和纽约日期，打印不再应用手机卡片布局，持仓移除重复展开。
- 保留厚安全边际门槛；短线阈值尚未通过收益验证，未实现账户级回撤控制。

# V6.8.1 - 2026-09-08

- 首页展示严格入场、合理估值、趋势子集三个统计和重点股票；区分研究上限与严格触发上限。
- 观察池按状态排序，手机显示卡片，A4 使用三列表格、重复表头及防拆行分页。
- 缺失日期不再输出 nan；显示香港或纽约时间，保留扫描时间及模型来源。
- 合并重复规则和诊断说明；完整扫描数据及筛选规则不变，模型版本保留 V6.8.0。
- 9月8日港股原快照样张由8页调整为4页；112项回归测试通过。

# V6.8.0

- 新增独立质量价格研究池、完整交易日趋势证据及公开前瞻观察日志。
- 缺失历史数据清除旧趋势；原厚安全边际门槛保持不变。
- 明确未验证收益及账户回撤限制。
...
- 公告补录新增归母与合并净利润独立字段；旺旺两年分别记录 4,335,565,000 / 4,328,415,000 元和 3,990,474,000 / 3,983,179,000 元，避免将合并利润映射到归母字段。估值优先采用明确的普通股股东净利润字段，缺失才沿用 Net Income。
- 真实冲突仍拒绝整只股票补录，诊断增加冲突字段、日期、供应商值和公告值，不自动覆盖。
- SBC 证据分为现金流表不可用、项目缺失、项目无值、部分年份缺失、历史不足和可用；零值不等于缺失，可用也不代表通过全部质量门槛。
- 港股扫描计划从 09:00 提前到 08:07，避开整点并预留全量扫描时间；开盘后生成的原定盘前报告及邮件标题标注延迟，CSV 与报告保留开始、完成及耗时。
- 91 项离线测试通过。旺旺实时抽查仍为空表，只验证补录空数据路径，尚未确认线上完整报表与公告所有字段匹配；调度提前不能保证 GitHub 准点执行。

## v6.7.3 - 2026-09-06

- 新增数据源年度现金流可用状态，以及 OCF、资本开支和 SBC 的缺失项目/年份诊断；诊断 CSV 保留期间、入场阻碍和公告补录审核记录。
- 新增 `data/hk_statement_evidence.json` 经人工核实的年度合并财报补录入口，在原币种换汇前填补缺失数据。严格核验全年期间、公布/审核日期、币种、单位、出处和必需字段，冲突时整只股票补录回滚并阻止入场。
- 补录文件变动使港股缓存失效，旧快照价格刷新也须先重扫；不改变估值倍数和折价门槛。
- 81 项离线测试通过，包括空表补录整条链路、换汇一次、缺失 SBC、来源冲突、期间和单位错误、缓存失效及历史不足拦截。
- 0151.HK、0552.HK、2555.HK 的实时年度现金流抽查均为空。已依据港交所年报逐项核对并录入旺旺 2024/25 与 2023/24 两年收入、利润和现金流；未核实的 SBC 省略并继续拦截，未验证全池覆盖率提升。详见 `docs/STATEMENT_EVIDENCE.md`。

## v6.7.2 - 2026-09-06

- 港股四季度合计不可用时尝试数据源明确标注的十二个月报表；核对最新日期、完整项目和新鲜度，失败保留年报并解释原因，限流停止后续请求。缺失的十二个月偿债指标不拼接年报值。
- 周期现金流基数改取近期、三年、五年均值中的较低值，采用原行业配置且最高 8 倍；移除基准中的额外 40%/85% 固定折减，保留压力情景、周期股 50% 折价与质量门槛。这是参数调整，尚无收益验证。
- 非正历史现金流均值归为风险，不再误报为波动率数据缺失。
- 报告展示三个触发上限、当前限制和财报日期；较远候选用简表，完整结果保留在 CSV；增加缺失及年报回退覆盖统计。
- 入场规则版本升至 3，版本切换首次观察不记为价格穿越；事件记录新增来源与触发价拆解。
- 新增 22 项测试，全套 68 项离线测试通过。两只港股实时抽查返回空报表，仅验证安全回退；报告预览沿用旧扫描值。详见 `docs/V6_7_2_PERIODS_AND_VALUATION.md`。

## v6.7.1 - 2026-09-06

- 根据实际扫描报告，将经营估值与资产折价参考分组；保留非正经营估值否决，纯资产结果转专门复核。
- 修复年报末尾空列误判 SBC 证据缺失，真实缺口与非连续年份仍拦截。
- 修复中断扫描生成报告后覆盖完整公开状态的问题；下一次加载回退完整快照。
- 报告使用严格入场统计与两列个股卡片，明确等待幅度、溢价、风险与缺失证据，移除普通扫描空回放收益和误导性接近列表。
- 新增 14 项回归测试，全套 46 项通过。实时数据源尚未验证成功，未验证策略收益。详见 `docs/SCAN_REPORT_FIXES.md`。
...
- 周期股估值进一步退守资产底线：周期行业不再使用 DCF、PE 或正常化 FCF 高弹性估值，只保留资产加低倍现金流、NCAV 和有形账面价值折价。
- 控股公司折价改为算法判定：长期投资资产/总资产超过 40% 时自动触发控股公司折价，并在数据质量备注中记录投资资产占比。
- yfinance 401/429、Unauthorized、Too Many Requests 触发时立即中断当前 ticker 后续财报请求，标记 `YAHOO_RATE_LIMIT`，降低 GitHub Actions 超时风险。
- 历史价格压力测试顶部警示文案升级，明确说明 Lookahead Bias 和“不是历史真实机会”。

## v6.5.8 - 2026-05-23

- 美股股票池过滤进一步剔除债券、票据、次级债、信托、优先股、含 `%` 固收类证券，降低非普通股财务分子分母错配风险。
- 估值前校验 `quoteType`，非 `EQUITY` 证券直接标记为 `SKIP`；公司名称疑似债券/优先股/封闭基金时也跳过估值。
- 港股新增低价与低成交金额熔断：股价低于 HK$1 或 10 日均成交金额低于 HK$500 万会触发评级封顶，二者同时出现时标记为流动性陷阱。
- 港股控股/综合企业增加结构性折价，疑似控股公司估值乘以 0.65，避免把底层资产价值全额传导给普通股股东。
- 周期股模型扩展农业/农产品周期，周期股 FCF 基数更保守，并加入高 FCF Yield 叠加收入/毛利下滑的利润反转风险标记。
- 历史价格压力测试新增醒目的未来函数警示；若当前财务数据套用历史价格后出现异常低估值倍数，会标记 `DATA_MISMATCH` 并停止评级。
