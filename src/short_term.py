...
def update_journal(journal, plans, frames, market, now=None):
    ...
    active = {(t.get('policy', POLICY),t['ticker']) for t in trades if t['state'] in {'PENDING', 'OPEN', 'DATA_REVIEW'}}
    for p in plans:
        key = (p.get('policy', POLICY),p['ticker'])
...
def ledger_html(trades):
    ...
    for policy,label in STRATEGIES.items():
        group = [t for t in closed if t.get('policy',POLICY)==policy]
        outcome = f'平均逐笔扣费收益 {np.mean([t["net_return"] for t in group]):+.2%}' if group else '尚无已结束样本'
        parts.append(f'<p>{label}：结束 {len(group)}笔；{outcome}。</p>')
    ...
    for t in active[:5]:
        ...
        parts.append(f'<p><b>{escape(t["ticker"])}</b> · {STRATEGIES.get(t.get("policy",POLICY),"未知策略")} · {escape(reason)}；止损参考 {t["stop"]:.2f}，'
...
def annotate_existing_positions(plans, trades):
    ...
    active = {(t.get('policy',POLICY),t['ticker']):t for t in trades if t['state'] in {'PENDING','OPEN','DATA_REVIEW'}}
...
def report_html(market, state_dir=None, now=None):
    ...
    try:
        ...
        return intro+f'<p>行情截至 {escape(data["signal_date"])}；短线检查完成 {observed_label}；实际检查 {len({p["ticker"] for p in data["plans"]})}只 / {len(data["plans"])}策略项。{escape(summary)}</p>'+coverage_text+(''.join(cards) or '<p>本轮没有可展示的触发计划。</p>')+watch+display_note+ledger+'<p class="sub">完整清单与逐笔模拟记录见 short-term 工作流产物。模拟收益不等于账户收益，尚不能评估20%账户回撤约束。</p>'
